...
namespace Lima
{
  public class SampleApp : TouchApp
  {
    ...
    {
      var window = new TouchView(ViewDirection.Row);
      var windowBar = new TouchWindowBar("Sample App");
      // var windowBar = new TouchView(TouchView.ViewDirection.Row);
      ...
      var col2 = new TouchView();
      // col2.Scale=new Vector2(1, 1);
      // col2.Border=new Vector4(2);

      var header1 = new TouchLabel("Column 1\nMultiline Test", 0.6f);

      var tabs = new TouchSwitch(new string[] { "Tab 1", "Tab 2", "Tab 3", "Tab 4" }, 0, (int v) =>
      {
        // Sandbox.Game.MyVisualScriptLogicProvider.SendChatMessage($"{v}", "SampleApp");
      });

      var labelSlider = new TouchLabel("A Touch Slider");
      labelSlider.Margin = Vector4.UnitY * 8;
      var slider = new TouchSlider(-100, 100, (float v) =>
      {
        // Sandbox.Game.MyVisualScriptLogicProvider.SendChatMessage($"{v}", "SampleApp");
      });
      slider.Value = 50f;
      var labelSliderRange = new TouchLabel("Range Slider");
      labelSliderRange.Margin = Vector4.UnitY * 8;
      var sliderRange = new TouchSliderRange(-100, 100, (float v, float v2) =>
      {
      ...
      var selector = new TouchSelector(names, (int i, string color) =>
      {
      ...
      var button = new TouchButton("A Touch Button", () =>
      {
      ...
      var textField = new TouchTextField("", (string text, bool wasCanceled) =>
      {
        if (!wasCanceled)
          Sandbox.Game.MyVisualScriptLogicProvider.SendChatMessage(text, "SampleApp");
      });

      var checkboxView = new TouchView(ViewDirection.Row);
      ...
      checkboxView.Gap = 4;
      var checkboxLabel = new TouchLabel("Checkbox", 0.5f, TextAlignment.RIGHT);
      var checkbox = new TouchCheckbox((bool v) =>
      {
        Sandbox.Game.MyVisualScriptLogicProvider.SendChatMessage($"{v}", "SampleApp");
      });
